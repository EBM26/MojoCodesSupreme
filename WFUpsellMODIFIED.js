...
$('body').on('change', '.selectQBottom', function() {
  ...
  currentItems.forEach(function(el, i) {
    if (el.id === Number(id)) {
      el.quantity = val;
      var price = `$${(14.95 * el.quantity).toFixed(2)} + $4.95 S&P`;
      priceEl.text(price);
    }
  });
