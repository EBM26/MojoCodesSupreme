<style>
  .starter-pack, .notOnSuper{display: none;}
  .btnActive a, .btnChoice:hover a{background-color: #02C39A !important; color: #ffffff !important;}
</style>
<script>
$(function(){
...
function getPageOffersByGroup(group) {
  ...
  return ___pageOffers
	...
	if (offer.trackingName == "14-Day GASF" || offer.trackingName.includes("14-Day GASF")) {	
		$('.product-title, .notOnSuper').hide();	
		$('.superImage, .free-14-bagSuper').show();	
		$('.pay-today-lbl').html("<b>$29.95 + Free Shipping</b>");	
		$('.bigPic img').attr("src","conjure_resource(e89310def03d4d83933ffcfc9d6b7e42)");
		if (isDeluxe) {	
		...
		$('.superImage, .free-14-bagSuper').hide();
		$('.product-title').html("FAMILY PACK");	
		$('.pay-today-lbl').html("Pay only $8.95 S&amp;P Today");	
		$('.bigPic img').attr("src","conjure_resource(c648c6ff3a30453b901510fabe22f34a)");			
		...
		$('.bigPic img').attr("src","conjure_resource(b17c3f1f6b454ab6aa4919473f24ce73)");	
		if (isDeluxe) {	
